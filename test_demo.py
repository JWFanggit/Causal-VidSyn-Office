...
IMAGE_PATH = "/3_005_frame_16.jpg"
MAP_PATH = "/0240_map.png"
TEXT_PROMPT = "the ego-car hits a car"
...
map_source, map = load_image(MAP_PATH)
map_source = cv2.resize(map_source, (224, 224), interpolation=cv2.INTER_LINEAR)
map_boxes = get_map_boxes(map_source)
map_boxes=convert_to_image_scale(map_boxes,)
map_boxes = map_boxes.unsqueeze(0).to(dtype=torch.float32)
...
print(logits)
print(phrases)
result=all(word in TEXT_PROMPT for word in phrases )
print(result)

annotated_frame = annotate(image_source=image_source, boxes=boxes, logits=logits, phrases=phrases)
annotated_frame1 = annotate(image_source=map_source, boxes=map_boxes, logits=logits, phrases=phrases)
cv2.imwrite("", annotated_frame)
cv2.imwrite("",annotated_frame1)
iou=compute_iou_between_boxes(tuple(boxes.squeeze(0).tolist()),tuple(map_boxes.squeeze(0).tolist()))
print(iou)
