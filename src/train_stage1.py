...
import gc
import copy
from dada import DADA2KS3,DADA2KS3_double
...
from V2V_Pipeline import TextToVideoSDPipeline
...
from losss import CosineDifferenceLoss
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    image_encoder = CLIPVisionModel.from_pretrained(pretrained_model_path, subfolder="image_encoder")
    ...
    train_dataset = DADA2KS3_double(root_path=r"/media/work/My Passport/CAPDATA", interval=1, phase="train")
    val_dataset = DADA2KS3(root_path=r"/media/work/My Passport/CAPDATA", interval=1, phase="val")
    ...
    def finetune_unet(batch, train_encoder=False):
        ...
        flip_pixel_values=batch["flip_pixel_values"].to(device, dtype=weight_dtype)
        if cache_latents:
            latents = tensor_to_vae_latent(pixel_values, vae)
            flip_latents=tensor_to_vae_latent(flip_pixel_values,vae)
        else:
            latents = pixel_values
            flip_latents= flip_pixel_values
        # Get video length
        video_length = latents.shape[2]
        # Sample noise that we'll add to the latents
        noise = sample_noise(latents, offset_noise_strength, use_offset_noise)
        p_noise = sample_noise(flip_latents, offset_noise_strength, use_offset_noise)
        bsz = latents.shape[0]
        ...
        p_noisy_latents = noise_scheduler.add_noise(flip_latents,p_noise, timesteps)
        # noisy_latents_map=noise_scheduler.add_noise(map_latents, noise, timesteps)
        ...
        for i in range(len(selected_text)):
            ...
            texts.append(text)
        p_selected_text = batch["p_prompt"]
        p_texts = []
        for i in range(len(selected_text)):
            text = tokenizer(
                p_selected_text[i], max_length=tokenizer.model_max_length, padding="max_length",
                truncation=True,
                return_tensors="pt"
            ).input_ids[0]
            p_texts.append(text)
        texts = torch.stack(texts).to(device)
        p_texts=torch.stack(p_texts).to(device)
        encoder_hidden_states = text_encoder(texts)[0]
        p_encoder_hidden_states=text_encoder(p_texts)[0]
        ...
        p_detached_encoder_state = p_encoder_hidden_states.clone().detach()
        p_trainable_encoder_state =p_encoder_hidden_states.clone()
        # for i in range(2):
        ...
        p_encoder_hidden_states = (
            p_detached_encoder_state if should_detach else p_trainable_encoder_state
        )
        difference_loss=CosineDifferenceLoss()
        model_pred= unet(noisy_latents, timesteps, encoder_hidden_states=encoder_hidden_states,
                                            bbx=None, fusion=None)
        p_model_pred= unet(p_noisy_latents, timesteps, encoder_hidden_states=p_encoder_hidden_states,
                                            bbx=None, fusion=None)
        loss0=difference_loss(model_pred,p_model_pred)
        loss1 = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
        loss2 = F.mse_loss(p_model_pred.float(), p_noise.float(), reduction="mean")
        loss_add=loss1+loss2+0.2*loss0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/lora_training_stage1.yaml")
