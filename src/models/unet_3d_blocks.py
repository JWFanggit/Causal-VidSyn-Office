...
from attentions import Transformer2DModel
from attentions import TransformerTemporalModel,TemporalConvLayer
...
def custom_checkpoint(module, mode=None):
    ...
    if mode == 'temp_conv':
        def custom_forward(hidden_states, num_frames=None, bbx=None, fusion=None, map=None):
...
def transformer_g_c(transformer, sample, num_frames, bbx, fusion, map):
    sample= g_c(custom_checkpoint(transformer, mode='temp'),
                                                      sample, num_frames, bbx, fusion, map, use_reentrant=False
                                                      )
...
def cross_attn_g_c(
    ...
    def ordered_g_c(idx):
        ...
        if idx == 0: return g_c(custom_checkpoint(attn, mode='attn'),
                                hidden_states, encoder_hidden_states, cross_attention_kwargs, bbx, fusion, map,
                                use_reentrant=False
                                )

        # Temporal Self and CrossAttention
        if idx == 1: return g_c(custom_checkpoint(temp_attn, mode='temp'),
                                hidden_states, num_frames, bbx, fusion, map, use_reentrant=False)
    ...
    if not inverse_temp:
        im_q_out = un_im_q_out = un_im_do_out = None
        for idx in [0, 1, 2, 3]:
            # hidden_states = ordered_g_c(idx)
            if idx == 1:
                hidden_states, im_q_out, un_im_q_out, un_im_do_out = ordered_g_c(idx)
            else:
                hidden_states = ordered_g_c(idx)
        return hidden_states, im_q_out, un_im_q_out, un_im_do_out
    else:
...
class UNetMidBlock3DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        for attn, temp_attn, resnet, temp_conv in zip(
            ...
            if self.gradient_checkpointing and fusion is not None:
                hidden_states, im_q_out, un_im_q_out, un_im_do_out = cross_attn_g_c(
                    attn,
                    temp_attn,
                    resnet,
                    temp_conv,
                ...
                if num_frames > 1:
                    hidden_states, im_q_out, un_im_q_out, un_im_do_out = temp_attn(hidden_states, num_frames=num_frames,
                                                                                   bbx=bbx, fusion=fusion, map=map)
...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def forward(
        ...
        return hidden_states, output_states, im_q_out, un_im_q_out, un_im_do_out
