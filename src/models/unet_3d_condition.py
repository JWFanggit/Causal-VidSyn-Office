...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.P_QA=P_QA(query_dim=1024,num_class1=5,num_class2=4)
    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            ...
            map=map)
                if video is not None:
                    video=torch.cat([im_q_out, un_im_q_out],dim=1)
                else:
                    video=None
            else:
        ...
        if self.mid_block is not None:
            sample,im_q_out, un_im_q_out, un_im_do_out= self.mid_block(
                sample,
                emb,
                encoder_hidden_states=encoder_hidden_states,
                attention_mask=attention_mask,
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                sample,im_q_out, un_im_q_out, un_im_do_out = upsample_block(
                    hidden_states=sample,
                    temb=emb,
        ...
        # post-process_qa
        out1,out2,out3= self.P_QA(im_q_out, un_im_q_out, un_im_do_out, question,r_option, non_option,answer)
        ...
        return UNet3DConditionOutput(sample=sample),out1, out2, out3
